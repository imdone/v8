...
#undef V
      ...
      default:
        if (IsIdentifierStart(ch)) {
          ...
          // TODO (bradnelson): Support unicode (probably via UnicodeCache). id:136 gh:137
          token_ = kParseError;
        }
        return;
    }
...
void AsmJsScanner::Rewind() {
  ...
  // TODO (bradnelson): Currently rewinding needs to leave in place the id:169 gh:170
...
void AsmJsScanner::ConsumeNumber(uc32 ch) {
  ...
  for (;;) {
    ...
    if ((ch >= '0' && ch <= '9') || (ch >= 'a' && ch <= 'f') ||
      ...
      // TODO (bradnelson): Test weird cases ending in -. id:150 gh:151
      if (ch == '.') {
        has_dot = true;
      }
      number.push_back(ch);
  ...
  if (std::isnan(double_value_)) {
    ...
    // TODO (bradnelson): Check if this happens often enough to be a perf id:193 gh:194
    // problem.
    if (number[0] == '.') {
