...
// TODO (jbramley): We can't put this inline in the class because things like id:110 gh:111
...
bool InstructionSequence::IsInlineData() const {
  ...
  // TODO (all): If we extend ::InlineData() to support bigger data, we need id:64 gh:65
  // to update this method too.
}


// TODO (jbramley): We can't put this inline in the class because things like id:97 gh:98
// xzr and Register are not defined in that header. Consider adding
// instructions-arm64-inl.h to work around this.
uint64_t InstructionSequence::InlineData() const {
  DCHECK(IsInlineData());
  uint64_t payload = ImmMoveWide();
  // TODO (all): If we extend ::InlineData() to support bigger data, we need id:139 gh:140
  // to update this method too.
  return payload;
}
